...
<section class="content">
		 ...
		 <!-- Basic Forms -->
		  <div class="box">
			<div class="box-header with-border">
			  <h4 class="box-title">Modifier module assignée</h4>
 ...
 <form method="post" action="{{ route('update.assign.class',$editData[0]->semestre_id) }}">
	 	@csrf
					  <div class="row">
						<div class="col-12">
						<div class="add_item">
	...
	<div class="controls">
	 <input type="text" name="semestre_id"  class="form-control" disabled
		 value="{{ $editData['0']['semestre']['name'] }}">
<!--{{ $editData['0']['semestre']['name'] }}-->
	 </div>
          </div> <!-- // end form group -->
   ...
   <div class="form-group">
	...
	<div class="controls">
	 <select name="module_id[]" required="" class="form-control">
		...
		<option value="{{ $class->id }}" {{ ($edit->module_id == $class->id)? "selected": ""  }}>{{ $class->name }}</option>
		@endforeach	 
		</select>
	 </div>
          </div> <!-- // end form group -->
