	...
	<a href="{{ route('student.shift.add') }}" style="float: right;" class="btn btn-rounded btn-success mb-5"> Add Student Shift</a>			  
		...
		<tbody>
			...
			<tr>
				...
				<td> {{ $shift->name }}</td>				 
				<td>
<a href="{{ route('student.shift.edit',$shift->id) }}" class="btn btn-info">Modifier</a>
<a href="{{ route('student.shift.delete',$shift->id) }}" class="btn btn-danger" id="delete">Supprimer</a>
